...
import static ratpack.rx.RxRatpack.publisher;

public class ChunkedObservableRenderer extends RendererSupport<Observable<?>> {
    ...
    public static void doRender(Context ctx, Observable<?> observable) {
        ctx.render(chunkedJsonList(ctx, publisher(observable)));
    }
}
