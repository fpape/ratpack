...
class ObservableRendererSpec extends RatpackGroovyDslSpec {
    def setup() {
    ...
    def "a observable success with single element is rendered as json structure"() {
        ...
        handlers {
            get { ctx ->
                ...
                ObservableRenderer.doRender(ctx, obs)
            }
        }
    ...
    def "a observable error triggers error status"() {
        ...
        handlers {
            get { ctx ->
                Observable<?> obs = Observable.error(new UnsupportedOperationException())
                ObservableRenderer.doRender(ctx, obs)
            }
        }
